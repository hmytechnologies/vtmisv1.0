...
if(isset($_REQUEST['action_type']) && (!empty($_REQUEST['action_type'])))
{
	...
	if($_REQUEST['action_type']=='addBasicInfo')
	{
        ...
        if( $db->isFieldExistMult($table,'schoolCode',$id_school,'staffEmail',$_POST['email']) )
		{
			header("location:index3.php?sp=manage_staff&msg=exist");
		}
		else
		{
			...
			$insertStaff = $db->insert($table,$staffData);
       		header("Location:index3.php?sp=manage_staff&msg=inserted");
		}
    }
}
...
if (isset($_POST['DoEdit'])){
	...
	header("location:index3.php?sp=manage_staff&msg=updated");
}
...
if (isset($_GET['block'])){
	...
	$progData = array(
			'status'=>0,'createdBy'=>1,
	);
	$conditions=array('teacherCode'=>$dId);
	$update=$db->update($table, $progData, $conditions);
	header("location: index3.php?sp=manage_staff&msg=blocked");
}
...
if (isset($_GET['unblock'])){
	...
	$conditions=array('teacherCode'=>$dId);
	$update=$db->update($table, $progData, $conditions);
	header("location: index3.php?sp=manage_staff&msg=unblocked");
}
?>