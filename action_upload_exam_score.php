...
if(isset($_REQUEST['action_type']) && !empty($_REQUEST['action_type'])){
    if($_REQUEST['action_type'] == 'add'){
        ...
        if($boolStatus)
        {
            header("Location:index3.php?sp=import_score&cid=".$db->encrypt($courseID)."&sid=".$db->encrypt($semesterID)."&bid=".$db->encrypt($batchID)."&msg=succ");
        }
        else
        {
            header("Location:index3.php?sp=import_score&cid=".$db->encrypt($courseID)."&sid=".$db->encrypt($semesterID)."&bid=".$db->encrypt($batchID)."&msg=unsucc");
        }
     ...
     header("Location:index3.php?sp=import_score&cid=".$db->encrypt($courseID)."&sid=".$db->encrypt($semesterID)."&bid=".$db->encrypt($batchID)."&msg=error");
 }