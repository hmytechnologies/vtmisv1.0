...
try {
    ...
    if (isset($_REQUEST['action_type']) && !empty($_REQUEST['action_type'])) {
        ...
        if ($_REQUEST['action_type'] == 'add') {
            ...
            if ($examCategoryID == 2) {
                foreach($_POST['examNumber'] as $key=>$reg)
                {
                    ...
                    //foreach ($_POST['examNumber'] as $reg => $regNumber) {
                    // foreach ($_POST['score'] as $sc => $examScore) {
                    //if ($reg == $sc) {
                ...
                foreach ($_POST['regNumber'] as $reg => $regNumber) {
                    foreach ($_POST['score'] as $sc => $examScore) {
                        if ($reg == $sc) {
                ...
                for ($x = 1; $x <= $number_student; $x++) {
                    ...
                    if (empty($err)) {
                        ...
                        /* $finalResultData=array(
                         ...
                         $final_condition = array('examNumber' =>$regNumber,'semesterSettingID'=>$semesterID,'courseID'=>$courseID,'examCategoryID'=>$examCategoryID);
                         $final_update = $db->update("final_result",$finalResultData,$final_condition);*/
