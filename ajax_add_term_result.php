...
if($_POST['examCategoryID'])
{
    ...
    ?>
    <div class="col-lg-6">
        <form name="" method="post" action="action_term_exam_score.php">
            <?php
            ...
            <table id="onlydata" border=1 class="table table-striped table-bordered table-condensed">
                ...
                ?>
                    <tr>
                        ...
                        $score = $db->getRows('exam_result', array('where' => array('examCategoryID' => $examCategoryID, 'regNumber' => $regNumber, 'courseID' => $courseID, 'academicYearID' => $academicYearID), ' order_by' => 'regNumber ASC'));
                        if (!empty($score)) {
                            foreach ($score as $sc) {
                                $present = $sc['present'];
                                ?>
